...
{
    public class DDialogueNode : DNode
    {
        ...
        public DDialogueNode(Vector2 position, int guid = -1) : base(position, guid)
        {
        ...
        private Foldout CreateDialogueText(int index)
        {
            ...
            var animationField = new ObjectField() { objectType = typeof(AnimationClip), value = text.Animation };
            animationField.RegisterValueChangedCallback(evt => UpdateTextAnimation(text, (AnimationClip)evt.newValue));
            ...
            textFoldout.Add(animationField);
            if (index != 0)
        ...
        private void UpdateTextAnimation(DText text, AnimationClip newAnimation) => text.Animation = newAnimation;
